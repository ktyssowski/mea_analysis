classdef BlockVectorData < Entry
    ...
    methods
        ...
        function Waveforms = GetSpikeV1Waveforms(...
            ...
            fHardwareChannelIndicies = typecast(fData(9,:), 'uint8');
            fHardwareChannelAchk = typecast(fData(10,:), 'uint8');
            fChannelIndicies = fChannelArray.LookupChannel(fHardwareChannelAchk, fHardwareChannelIndicies);
            load_channel_logical = ismember(fChannelIndicies, aChannelsToLoad);

            for iWave = 1 : fNumWaves
                if ~load_channel_logical(iWave)
                    %there's probably a better way to implement this loops without breaks but this will work for testing purposes
                    continue;
                end
                ...
                fChannelMapping = fChannelArray.Channels(fChannelIndicies(iWave));
                ...
                if fDesiredChannelsLut(fChannelIndicies(iWave)) ~= 0
